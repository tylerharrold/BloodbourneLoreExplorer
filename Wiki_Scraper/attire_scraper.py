...
url = "https://www.bloodborne-wiki.com/p/armor.html"
	...
	for row in rows:
		...
		for a in a_tags:
			if a:
				text = a.get_text()
				if 'Set' in text:
					continue # this is a link to a set page and we can ignore this
				else:
					link = a.get('href')
					if link:
						links.append(link)
...
for link in links:
	...
	# one item, the One-Eyed Iron Helm, is a unique case and will be entered manually
	if 'one-eyed-iron-helm' in link:
		print("Skipping one eyed iron helm...")
		continue
