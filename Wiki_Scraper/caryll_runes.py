...
# only the first 5 tables of this class are relevant to scrape
for i in range(5):
...
for link in links:
	...
	caryll_runes[name] = current_item

	print("nabbed item")

# now that we've gotten all the items, dump them as a json file
with open('JSON/caryll_runes.json' , 'w') as f:
	json.dump(caryll_runes , f)
